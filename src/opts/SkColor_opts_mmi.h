...
#ifndef SkColor_opts_MMI_DEFINED
#define SkColor_opts_MMI_DEFINED

#include "loongson-mmintrin.h"

#define ASSERT_EQ(a,b) SkASSERT(0xffff == _mm_movemask_epi8(_mm_cmpeq_epi8((a), (b))))

// Because no _mm_mul_epi32() in MMI, we emulate it here.
...
static inline  __m128i Multiply32_MMI(const __m128i& a, const __m128i& b) {
    ...
    __m128i r = _mm_unpacklo_epi32(_mm_shuffle_epi32_0020(r1),
                                   _mm_shuffle_epi32_0020(r2));
    return r;
}

static inline __m128i SkAlpha255To256_MMI(const __m128i& alpha) {
    return _mm_add_epi32(alpha, _mm_set1_epi32(1));
}

// See #define SkAlphaMulAlpha(a, b)  SkMulDiv255Round(a, b) in SkXfermode.cpp.
static inline __m128i SkAlphaMulAlpha_MMI(const __m128i& a,
                                           const __m128i& b) {
...
static inline __m128i SkAlphaMulQ_MMI(const __m128i& c, const __m128i& scale) {
...
// Fast path for SkAlphaMulQ_MMI with a constant scale factor.
static inline __m128i SkAlphaMulQ_MMI(const __m128i& c, const unsigned scale) {
    const __m128i mask = _mm_set1_epi32(0xFF00FF);
...
static inline __m128i SkFastFourByteInterp256_MMI(const __m128i& src, const __m128i& dst, const unsigned src_scale) {
...
static inline __m128i SkPMLerp_MMI(const __m128i& src, const __m128i& dst, const unsigned scale) {
    return SkFastFourByteInterp256_MMI(src, dst, scale);
}

static inline __m128i SkGetPackedA32_MMI(const __m128i& src) {
...
static inline __m128i SkGetPackedR32_MMI(const __m128i& src) {
    __m128i r = _mm_slli_epi32(src, (24 - SK_R32_SHIFT));
    return _mm_srli_epi32(r, 24);
}

static inline __m128i SkGetPackedG32_MMI(const __m128i& src) {
    __m128i g = _mm_slli_epi32(src, (24 - SK_G32_SHIFT));
    return _mm_srli_epi32(g, 24);
}

static inline __m128i SkGetPackedB32_MMI(const __m128i& src) {
    __m128i b = _mm_slli_epi32(src, (24 - SK_B32_SHIFT));
    return _mm_srli_epi32(b, 24);
}

static inline __m128i SkMul16ShiftRound_MMI(const __m128i& a,
...
static inline __m128i SkPackRGB16_MMI(const __m128i& r,
...
static inline __m128i SkPackARGB32_MMI(const __m128i& a, const __m128i& r,
                                        const __m128i& g, const __m128i& b) {
...
static inline __m128i SkPacked16ToR32_MMI(const __m128i& src) {
...
static inline __m128i SkPacked16ToG32_MMI(const __m128i& src) {
...
static inline __m128i SkPacked16ToB32_MMI(const __m128i& src) {
...
static inline __m128i SkPixel16ToPixel32_MMI(const __m128i& src) {
    __m128i r = SkPacked16ToR32_MMI(src);
    __m128i g = SkPacked16ToG32_MMI(src);
    __m128i b = SkPacked16ToB32_MMI(src);

    return SkPackARGB32_MMI(_mm_set1_epi32(0xFF), r, g, b);
}

static inline __m128i SkPixel32ToPixel16_ToU16_MMI(const __m128i& src_pixel1,
    ...
    __m128i d_pixel = SkPackRGB16_MMI(r, g, b);

    return d_pixel;
}

// Portable version is SkPMSrcOver in SkColorPriv.h.
static inline __m128i SkPMSrcOver_MMI(const __m128i& src, const __m128i& dst) {
    return _mm_add_epi32(src,
                         SkAlphaMulQ_MMI(dst, _mm_sub_epi32(_mm_set1_epi32(256),
                                                             SkGetPackedA32_MMI(src))));
}

// Fast path for SkBlendARGB32_MMI with a constant alpha factor.
static inline __m128i SkBlendARGB32_MMI(const __m128i& src, const __m128i& dst,
                                         const unsigned aa) {
    unsigned alpha = SkAlpha255To256(aa);
    __m128i src_scale = _mm_set1_epi16(alpha);
    // SkAlphaMulInv256(SkGetPackedA32(src), src_scale)
    __m128i dst_scale = SkGetPackedA32_MMI(src);
...
#undef ASSERT_EQ
#endif // SkColor_opts_MMI_DEFINED
