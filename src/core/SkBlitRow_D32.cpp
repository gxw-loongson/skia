...
// TODO(mtklein): can we do better in NEON than 2 pixels at a time?
    ...
    static void blit_row_s32_blend(SkPMColor* SK_RESTRICT dst,
        ...
        if (count >= 4) {
            ...
            while (count >= 4) {
                ...
                __m128i result = SkPMLerp_MMI(src_pixel, dst_pixel, src_scale);
    ...
    static void blit_row_s32a_blend(SkPMColor* SK_RESTRICT dst,
                                   const SkPMColor* SK_RESTRICT src,
                                   int count, U8CPU alpha) {
        SkASSERT(alpha <= 255);
        if (count <= 0) {
        ...
        if (count >= 4) {
            ...
            while (count >= 4) {
                ...
                __m128i result = SkBlendARGB32_MMI(src_pixel, dst_pixel, alpha);
                _mm_store_si128(d, result);
                s++;
                d++;
                count -= 4;
